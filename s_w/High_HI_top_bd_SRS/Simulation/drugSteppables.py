...
varii_1 = 2.25 #growth rate coefficient (was 0.75 - increased for faster growth)
varii_2 = 2.25
...
class MitosisSteppable_R(MitosisSteppableBase):
    ...
    CANCER_R_DIVIDE_VOLUME = 40  # CANCER_S uses 30
...
class DrugResponseSteppable(SteppableBasePy):
    ...
    - CANCER_S (sensitive): the first time local DRUG concentration exceeds
      DRUG_THRESHOLD, a one-time fate decision is made:
    ...
    - CANCER_R (resistant): completely ignores the DRUG field.

    Registration order matters: register AFTER VolumeParamSteppable so that
    the shrinkage assignment here overrides the growth term for dying cells.
    """
    ...
    def step(self, mcs):
        ...
        for cell in self.cell_list_by_type(self.CANCER_S):
            ...
            # ── One-time fate decision when drug threshold is first crossed ──
            if not cell_dict.get("drugHit", False) and not cell_dict.get("drugResisted", False):
                if drug_c > DRUG_THRESHOLD:
                    if random.random() < self.DRUG_DEATH_PROB:
                        ...
                        # Inherit volume/surface params appropriate for CANCER_R
                        cell.targetSurface = 20.0
                        cell.lambdaSurface = 2.0
                        # targetVolume and lambdaVolume will be set next step
                        # by VolumeParamSteppable; skip further processing here
                        continue
