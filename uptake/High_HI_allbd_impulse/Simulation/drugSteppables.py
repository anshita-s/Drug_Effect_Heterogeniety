...
# --- Drug delivery (impulse / pulse) parameters ---
# Replaces the old continuous XML boundary source (constant value = 10.0
# on the Y=Max plane only) with a periodic bolus delivered from all four
# domain boundaries (X=Min, X=Max, Y=Min, Y=Max).
DRUG_PULSE_VALUE    =60.0   # boundary concentration during a pulse (was 10.0, continuous)
DRUG_PULSE_DURATION = 20     # how many consecutive MCS the pulse stays "on"
DRUG_PULSE_PERIOD   = 2000   # MCS between the start of one pulse and the next
...
######################### Drug pulse (impulse) delivery ##########################
class DrugPulseSteppable(SteppableBasePy):
    """
    Delivers DRUG as a periodic impulse (bolus) from all four domain
    boundaries (X=Min, X=Max, Y=Min, Y=Max), instead of the old continuous
    constant-value boundary condition that used to be set in drug.xml
    (Y plane only, value = 10.0, held for the entire simulation).

    Behaviour:
      - For the first DRUG_PULSE_DURATION MCS of every DRUG_PULSE_PERIOD-MCS
        cycle (i.e. mcs % DRUG_PULSE_PERIOD < DRUG_PULSE_DURATION), all
        boundary voxels on all four sides are clamped to DRUG_PULSE_VALUE.
      - Outside that window the boundary voxels are left alone (not forced
        to 0), so whatever drug is already in the field simply diffuses and
        decays according to the DiffusionSolverFE settings in drug.xml.
        This produces a sharp bolus every DRUG_PULSE_PERIOD MCS rather than
        a constant source.

    Must run with frequency=1 so pulse timing/duration is exact in MCS.
    """
    ...
    def start(self):
        self.dim_x = self.dim.x
        self.dim_y = self.dim.y
        self.dim_z = self.dim.z

    def step(self, mcs):
        # Are we inside a pulse window of the current cycle?
        if (mcs % DRUG_PULSE_PERIOD) >= DRUG_PULSE_DURATION:
            return  # outside the pulse window: let the field evolve on its own

        field = self.field.DRUG

        # X = Min and X = Max boundaries (all y, all z)
        for y in range(self.dim_y):
            for z in range(self.dim_z):
                field[0, y, z] = DRUG_PULSE_VALUE
                field[self.dim_x - 1, y, z] = DRUG_PULSE_VALUE

        # Y = Min and Y = Max boundaries (all x, all z)
        for x in range(self.dim_x):
            for z in range(self.dim_z):
                field[x, 0, z] = DRUG_PULSE_VALUE
                field[x, self.dim_y - 1, z] = DRUG_PULSE_VALUE