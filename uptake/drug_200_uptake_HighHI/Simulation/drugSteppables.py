...
class CellLayoutSteppable(SteppableBasePy):
    ...
    def start(self):  
        ...
        #COLLAGEN-I
        for x in range(5,195,4):
            for y in range(5,195,4):
                self.cell_field[x:x+2,y:y+2,0] = self.new_cell(self.COI)
                
        #LAMININ      
        for x in range(90,110,3):
     ...
     #   for x in range(92,108,4):
     #       for y in range(92,100,4):
     #           self.cell_field[x:x+4, y:y+4, 0] = self.new_cell(self.CANCER_S)
               
        # Drug-RESISTANT subset (formerly CANCER_B)
     #   for x in range(92,108,4):
     #       for y in range(100,108,4):
     #           self.cell_field[x:x+4, y:y+4,0] = self.new_cell(self.CANCER_R)
                
        
        # # # High HI
        for x in it.chain(range(92,96,4), range(100,104,4)):
            for y in it.chain(range(92,96,4), range(100,104,4)):
                self.cell_field[x:x+4, y:y+4, 0] = self.new_cell(self.CANCER_S)
        for x in it.chain(range(96,100,4), range(104,108,4)):
            for y in it.chain(range(96,100,4), range(104,108,4)):
                self.cell_field[x:x+4, y:y+4, 0] = self.new_cell(self.CANCER_S)
...
class VolumeParamSteppable(SteppableBasePy):
    ...
    def start(self):
        ...
        # Margin (in pixels) from the lattice edge at which cancer cells get
        # deleted. Kept as a fixed margin rather than a fixed coordinate so
        # this steppable behaves consistently regardless of lattice size
        # (previously this was hardcoded as x/y > 97 or < 3, which silently
        # assumed a 100x100 lattice and broke on larger lattices).
        self.boundary_margin = 3
    ...
    def step(self,mcs):
        ...
        for cell in self.cell_list_by_type(self.CANCER_S):
            ...
            x_max = self.dim.x - self.boundary_margin
            y_max = self.dim.y - self.boundary_margin
            if (int(round(cell.xCOM)) > x_max or int(round(cell.xCOM)) < self.boundary_margin or
                    int(round(cell.yCOM)) > y_max or int(round(cell.yCOM)) < self.boundary_margin):
